...
# (Header/contact is always first in the PDF)
...
def get_default_word_template_path() -> str:
    # Kept for compatibility; not used for defaults anymore, but you
    # can still reference the original Word resume if needed.
    return os.path.join(BASE_DIR, "Shamik_Basu_Data_Scientist_2026__Resume.docx")
...
def _safe_for_multicell(text: str, max_chunk: int = 40) -> str:
    """
    Ensure there are spaces at reasonable intervals so FPDF can wrap.
    Long continuous tokens (like very long URLs) are split into chunks.
    """
    safe_words: list[str] = []
...
def default_file_name() -> str:
    ...
    # Keep user-requested base name stable while avoiding overwrite by appending timestamp
    return f"Shamik_Basu_Resume_{timestamp}.pdf"
...
def _needs_prefill_experience(items: Any) -> bool:
    ...
    for it in items:
        if not isinstance(it, dict):
            return True
        # Older session states may only have highlights
...
def _href_url(url: str) -> str:
    """Escape URL for use inside \\href{...}{...} first argument."""
    if not url:
        return ""
...
def _build_skills_tex(skills: List[str]) -> str:
    ...
    # Single flowing line to match typical resume skills section
...
def generate_pdf(
    ...
    """
    Generate a PDF by filling a LaTeX template and compiling it.
    All sections are built from the passed-in data. Section order can be customized.
    """
    ...
    # Build body in user-specified order
    body_parts = []
    for sec_id in order:
    ...
    # Write filled TeX into dedicated tex subfolder under processed (ephemeral)
    base_name = os.path.splitext(os.path.basename(target_path))[0]
    tex_out_path = os.path.join(TEX_DIR, f"{base_name}.tex")
    with open(tex_out_path, "w", encoding="utf-8") as f:
        f.write(filled_tex)

    # Compile with xelatex; require LaTeX to be installed on the system
    try:
        subprocess.run(
            [
                "xelatex",
    ...
    # Move compiled PDF from tex folder into dedicated pdf folder
    compiled_pdf = os.path.join(TEX_DIR, f"{base_name}.pdf")
    if os.path.exists(compiled_pdf):
        os.replace(compiled_pdf, target_path)

    # Clean up intermediate LaTeX artifacts so only PDFs remain under processed/
...
def main() -> None:
    st.set_page_config(page_title="Shamik Resume Builder", page_icon="📄", layout="wide")
    st.title("Custom Resume Builder for Shamik Basu")
...
- Export as a **PDF** saved under `processed/pdf/Shamik_Basu_Resume_YYYYMMDD_HHMMSS.pdf`
"""
    )
    ...
    # Initialize session state for editable lists and section order
    ...
    # Half-page editor | half-page live PDF
    ...
    with col_main:
        ...
        # Sections in template order (summary, education, experience, projects, leadership, skills)
    ...
    with col_preview:
        ...
        # summary_lines comes from left column (Professional Summary expander)

        st.markdown("---")
        st.subheader("Generate / Preview PDF")

        # Live preview: regenerate a stable preview PDF on each run when we have projects selected
        preview_path = None
        if selected_projects:
            ...
            # Embed the preview in the right pane (half page wide + tall)
            try:
